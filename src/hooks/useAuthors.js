...
export function useAuthors() {
  ...
  useEffect(() => {
    let cancelled = false;
    listen('authors_progress', (event) => {
      if (!cancelled) setProgressMessage(event.payload);
  ...
  const loadAuthors = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await invoke('get_abs_authors');
  ...
  const loadDetail = useCallback(async (authorId) => {
    ...
    try {
      const result = await invoke('get_abs_author_detail', { authorId });
  ...
  const runAnalysis = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await invoke('analyze_authors_from_abs');
      setAnalysis(result);
      return result;
    } catch (err) {
      setError(typeof err === 'string' ? err : String(err));
  ...
  const fixDescriptions = useCallback(async (authorIds, force) => {
    ...
    setError(null);
    try {
      const result = await invoke('fix_author_descriptions_gpt', {
        authorIds,
        force: force || false,
      });
  ...
  const pushToAbs = useCallback(async () => {
    ...
    try {
      ...
      if (changeEntries.length > 0) {
        ...
        const result = await invoke('push_author_changes_to_abs', { items });
      ...
      for (const [primaryId, secondaries] of mergeEntries) {
        try {
          await invoke('merge_abs_authors', {
            primaryId,
