...
export function useAbsCache() {
  ...
  const refreshCache = useCallback(async () => {
    ...
    try {
      ...
      unlisten = await listen('abs_cache_progress', (event) => {
        setRefreshProgress(event.payload);
      });

      // Refreshing ABS cache
      const result = await invoke('refresh_abs_cache');
  ...
  const getCachedItems = useCallback(async () => {
    try {
      return await invoke('get_cached_items');
    } catch (error) {
  ...
  const getCachedItem = useCallback(async (id) => {
    try {
      return await invoke('get_cached_item', { id });
    } catch (error) {
  ...
  const getCachedItemFiles = useCallback(async (id) => {
    try {
      return await invoke('get_cached_item_files', { id });
    } catch (error) {
  ...
  const getCachedItemChapters = useCallback(async (id) => {
    try {
      return await invoke('get_cached_item_chapters', { id });
    } catch (error) {
  ...
  const searchCachedItems = useCallback(async (query) => {
    try {
      return await invoke('search_cached_items', { query });
    } catch (error) {
  ...
  const clearCache = useCallback(async () => {
    try {
      await invoke('clear_abs_full_cache');
      setCacheStatus({
        isLoaded: false,
        isStale: true,
        stats: null,
