...
export function ScannerPage({ onNavigateToSettings }) {
  const {
  ...
  const handleGenreCleanup = async () => {
    ...
    if (!config?.openai_api_key) {
      ...
      try {
        ...
        console.log(`✅ Cleaned genres for ${result.count} books (static rules)`);
      } catch (error) {
    ...
    console.log(`🤖 GPT Genre Cleanup for ${selectedGroups.length} books...`);
    ...
    for (let i = 0; i < selectedGroups.length; i += batchSize) {
      ...
      try {
        ...
        const result = await invoke('cleanup_genres_with_gpt', { books, config });
    ...
    console.log(`✅ GPT Genre Cleanup: ${cleanedCount} cleaned, ${unchangedCount} unchanged, ${failedCount} failed`);
  ...
  const handleFixTitles = async () => {
    ...
    console.log(`📚 Fixing titles with GPT for ${selectedGroups.length} books...`);
      ...
      const results = await Promise.allSettled(
        chunk.map(async (group) => {
          try {
            ...
            const result = await invoke('resolve_title', {
              request: {
          ...
          if (batchResult && batchResult.status === 'fulfilled') {
            ...
            if (result.success && result.result) {
              ...
              if (r.confidence < 70 && r.suggested_title) {
                console.log(`⚠️ Low confidence (${r.confidence}%) for "${g.group_name}"`);
                console.log(`   GPT title: "${r.title}"`);
                console.log(`   Suggested: "${r.suggested_title}" (from ${r.suggestion_source})`);
              }
    ...
    console.log(`✅ Title fixing complete: ${successCount} success, ${failedCount} failed`);
    batch.end('titles');
  };
  ...
  const handleFixSubtitles = async () => {
    ...
    console.log(`📖 Fixing subtitles for ${selectedGroups.length} books...`);
    ...
    for (let i = 0; i < selectedGroups.length; i += batchSize) {
      ...
      try {
        ...
        const result = await invoke('fix_subtitles_batch', { books, config, force: forceFresh });
        ...
        if (result.results && result.results.length > 0) {
          setGroups(prevGroups => {
            return prevGroups.map(g => {
              ...
              console.log(`📖 ${g.metadata?.title}: subtitle set to "${subResult.subtitle}" (source: ${subResult.source})`);
    ...
    console.log(`✅ Subtitle Fix: ${fixedCount} fixed, ${skippedCount} skipped, ${failedCount} failed`);
  ...
  const handleFixAuthors = async () => {
    ...
    console.log(`👤 Fixing authors for ${selectedGroups.length} books...`);
    ...
    for (let i = 0; i < selectedGroups.length; i += batchSize) {
      ...
      try {
        ...
        const result = await invoke('fix_authors_batch', { books, config, force: forceFresh });
        ...
        if (result.results && result.results.length > 0) {
          setGroups(prevGroups => {
            return prevGroups.map(g => {
              ...
              console.log(`👤 ${g.metadata?.title}: author set to "${authResult.author}" (source: ${authResult.source})`);
    ...
    console.log(`✅ Author Fix: ${fixedCount} fixed, ${skippedCount} skipped, ${failedCount} failed`);
  ...
  const handleFixYears = async () => {
    ...
    console.log(`📅 Fixing years for ${selectedGroups.length} books...`);
    ...
    for (let i = 0; i < selectedGroups.length; i += batchSize) {
      ...
      try {
        ...
        const result = await invoke('fix_years_batch', { books, config, force: forceFresh });
        ...
        if (result.results && result.results.length > 0) {
          setGroups(prevGroups => {
            return prevGroups.map(g => {
              ...
              console.log(`📅 ${g.metadata?.title}: date set to "${yearResult.pub_date}" (source: ${yearResult.source})`);
    ...
    console.log(`✅ Year Fix: ${fixedCount} fixed, ${skippedCount} skipped, ${failedCount} failed`);
  ...
  const handleFixSeries = async () => {
    ...
    console.log(`📚 Fixing series via Audible + GPT for ${selectedGroups.length} books...`);
    ...
    for (let i = 0; i < selectedGroups.length; i += batchSize) {
      ...
      const results = await Promise.allSettled(
        chunk.map(async (group) => {
          try {
            const result = await invoke('resolve_series', {
              request: {
    ...
    console.log(`✅ Series fixing complete: ${successCount} success, ${failedCount} failed`);
    batch.end('series');
  };
  ...
  const handleLookupAge = async () => {
    ...
    console.log(`🔍 Looking up age ratings for ${selectedGroups.length} books...`);
    ...
    for (let i = 0; i < selectedGroups.length; i += batchSize) {
      ...
      const results = await Promise.allSettled(
        chunk.map(async (group) => {
          try {
            const result = await invoke('resolve_book_age_rating', {
              request: {
                title: group.metadata?.title || group.group_name,
                author: group.metadata?.author || '',
    ...
    console.log(`✅ Age lookup complete: ${successCount} success, ${failedCount} failed`);
    batch.end('age');
  };
  ...
  const handleLookupISBN = async () => {
    ...
    if (skippedIsbn > 0) console.log(`🔍 Skipping ${skippedIsbn} books with existing ISBN/ASIN (use Force Fresh to override)`);
    ...
    console.log(`🔍 Looking up ISBN/ASIN for ${booksToProcess.length} books...`);
        ...
        chunk.map(async (group) => {
          try {
            ...
            if (gathered && (gathered.isbn || gathered.asin)) {
              console.log(`📡 ${group.metadata?.title}: using pre-fetched ISBN/ASIN`);
              return { groupId: group.id, result: { success: true, isbn: gathered.isbn, asin: gathered.asin, source: 'gather-phase' } };
            }
            const result = await invoke('lookup_book_isbn', {
              request: {
                title: group.metadata?.title || group.group_name,
                author: group.metadata?.author || '',
              }
    ...
    console.log(`✅ ISBN lookup complete: ${successCount} success, ${failedCount} failed, ${skippedIsbn} skipped`);
  ...
  const handleMetadataResolution = async () => {
    ...
    if (skipped > 0) console.log(`📝 Skipping ${skipped} books with complete metadata (use Force Fresh to override)`);
    ...
    console.log(`📝 Consolidated metadata resolution for ${booksToProcess.length} books...`);
    ...
    try {
      const result = await invoke('resolve_metadata_batch', { books, config });
      ...
      console.log(`✅ Metadata resolution: ${processed} processed, ${failed} failed`);
  ...
  const handleDescriptionProcessing = async () => {
    ...
    if (skippedDesc > 0) console.log(`📖 Skipping ${skippedDesc} books with good descriptions (use Force Fresh to override)`);
    ...
    console.log(`📝 Consolidated description processing for ${booksToProcess.length} books...`);
    ...
    try {
      const result = await invoke('process_descriptions_batch', { books, config });
      ...
      console.log(`✅ Description processing: ${processed} processed, ${failed} failed`);
  ...
  const handleRunAll = async () => {
    ...
    console.log(`🚀 Phase 1: Gathering external data for all selected books...`);
    ...
    try {
      ...
      const gatherResult = await invoke('gather_external_data', { books: gatherBooks, config });
      ...
      console.log(`✅ Phase 1 complete: gathered data for ${dataMap.size} books`);
    } catch (error) {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      try {
        console.log(`📌 Step ${stepNum}/${totalSteps}: ${step.name}`);
        await step.fn();
        successCount++;
      } catch (error) {
    ...
    console.log(`✅ Run All complete: ${successCount} steps succeeded, ${failedCount} failed`);
  ...
  const handleAssignTagsGpt = async () => {
    ...
    console.log(`🤖 Assigning tags with GPT for ${selectedGroups.length} books...`);
    ...
    for (let i = 0; i < selectedGroups.length; i += batchSize) {
      ...
      try {
        ...
        const result = await invoke('assign_tags_with_gpt', { books, config });
    ...
    console.log(`✅ GPT Tag Assignment: ${successCount} success, ${failedCount} failed`);

    // Now run age rating lookup for the same books and append age tags
    console.log(`🔍 Now looking up age ratings for ${selectedGroups.length} books...`);
    ...
    console.log(`✅ Age rating lookup complete`);
  ...
  const handleGenerateDna = async () => {
    ...
    console.log(`🧬 Generating BookDNA for ${selectedGroups.length} books...`);
    ...
    try {
      ...
      const unlisten = await listen('dna-progress', (event) => {
        const { current, total, id, title, success, error } = event.payload;
      ...
      // Call batch API
      const results = await invoke('generate_book_dna_batch', {
        request: { items },
      });
      ...
      console.log(`✅ BookDNA generation: ${successCount} success, ${failedCount} failed`);
  ...
  const handleClassifyAll = async (includeDescription = true) => {
    ...
    if (skippedClassify > 0) console.log(`🤖 Skipping ${skippedClassify} books already classified (use Force Fresh to override)`);
    ...
    console.log(`🤖 Consolidated classification for ${booksToProcess.length} books...`);
    ...
    try {
      const result = await invoke('classify_books_batch', {
        books,
        includeDescription,
      ...
      console.log(`✅ Classification: ${successCount} processed, ${failedCount} failed`);
  ...
  const handleFixDescriptionsGpt = async () => {
    ...
    console.log(`📝 Fixing descriptions with GPT for ${selectedGroups.length} books...`);
    ...
    for (let i = 0; i < selectedGroups.length; i += batchSize) {
      ...
      try {
        ...
        const result = await invoke('fix_descriptions_with_gpt', { books, config, force: forceFresh });
          ...
          setGroups(prevGroups => {
            return prevGroups.map(g => {
              ...
              if (descResult.extracted_narrator) {
                ...
                changes++;
                console.log(`🎤 ${g.metadata?.title}: narrator set to "${descResult.extracted_narrator}"`);
              }
    ...
    console.log(`✅ Description Fix: ${fixedCount} fixed, ${skippedCount} already good, ${failedCount} failed`);
  ...
  const handleFixSingleIssue = useCallback((groupId, field, suggestedValue) => {
    ...
    console.log(`✅ Fixed ${field} for book ${groupId}`);
  }, [setGroups]);
  ...
  const handleApplyAuthorFixes = useCallback((fixes) => {
    ...
    clearValidation();
    console.log(`✅ Normalized ${fixes.length} author names`);
  ...
  const handleApplyValidationFixes = useCallback((fixes) => {
    ...
    clearValidation();
    console.log(`✅ Applied ${fixes.length} validation fixes`);
  }, [setGroups, clearValidation]);
  ...
  const handleApplySeriesFixes = useCallback(async (fixes) => {
    ...
    try {
      ...
      clearValidation();
      console.log(`✅ Applied ${fixes.length} series fixes`);
    } catch (error) {
  ...
  const handleCancelScan = useCallback(async () => {
    ...
    try {
      await invoke('cancel_series_scan');
    } catch (e) {
  ...
  const confirmBatchFix = useCallback(async () => {
    ...
    if (totalFixes > 0) {
      ...
      console.log(`✅ Applied ${totalFixes} total fixes (${appliedTypes.join(', ')})`);
    } else {
  ...
  const handleConfirmPush = async () => {
    ...
    // Check if these are ABS imports (no local files)
    const absImports = pushGroups.filter(g => g.files.length === 0);
    const localFiles = pushGroups.filter(g => g.files.length > 0);
