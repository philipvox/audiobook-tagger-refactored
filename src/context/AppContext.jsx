...
export function AppProvider({ children }) {
  ...
  const runValidation = useCallback(async (groupsToValidate) => {
    ...
    const unlisten = await listen('validation_progress', (event) => {
    ...
    try {
      const result = await invoke('scan_metadata_errors', { groups: groupsToValidate });
      ...
      setValidationStats({
        ...
        clean: result.total_scanned - result.books.length,
      });
  ...
  const runAuthorAnalysis = useCallback(async (groupsToAnalyze) => {
    ...
    try {
      const result = await invoke('analyze_authors', { groups: groupsToAnalyze });
  ...
  const runSeriesAnalysis = useCallback(async (groupsToAnalyze) => {
    ...
    // Listen for progress events from backend
    const unlisten = await listen('series_analysis_progress', (event) => {
    ...
    try {
      const result = await invoke('analyze_series_comprehensive', {
  ...
  const applySeriesFixes = useCallback(async (currentGroups, fixes) => {
    ...
    try {
      const updatedGroups = await invoke('apply_series_fixes', {
